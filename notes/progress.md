...
| Step                            | Status                   | Notes                                                                                                                                                                       |
| ------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Plane split (step 1)**        | Documented / may be done | Masked intensity TIFFs → `data/test segment embryo/`. See `[plane_split_napari_workflow.md](plane_split_napari_workflow.md)`. |
| **micro-SAM nuclei (step 2)**   | **Ready to run**         | `run_micro_sam_nuclei.py` — DAPI from segment → `[micro_sam_nuclei_workflow.md](micro_sam_nuclei_workflow.md)`. Prior labels may exist in `data/test_cell_labels/`. |
...
Raw inputs stay in `22A_E1_Wnt3/` (not in git). **Z-range on disk:** `z18`–`z112` (95 slices × 2 channels); empty slices below z18 and above z112 were removed.

Local outputs, TIFFs, `.npy`, and Zarr (`.zattrs`, `data/`, etc.) are listed in `.gitignore`.
...
| Launch SAM (step 2) | `scripts/run_micro_sam_nuclei.bat` (loads segment DAPI + Annotator 3d)       |
| Blank Annotator 3d  | `scripts/run_micro_sam_3d.bat`                                                   |
...
22A_E1_Wnt3/  (DAPI + Wnt channels)
    ...
    ├─ Step 2: micro-SAM nuclei  →  data/test_cell_labels/
    │
...
1. **Step 2 — micro-SAM nuclei** — `scripts/run_micro_sam_nuclei.bat` after step 1; save `committed_objects` → `data/test_cell_labels/`. See `[micro_sam_nuclei_workflow.md](micro_sam_nuclei_workflow.md)`.
2. **Step 3 — Embryo from background (DAPI + Wnt)** — see [`processing_steps.md`](processing_steps.md) § Step 3: segment on both channels, then 3D dilate (nuclei-only dilation does not cover Wnt). Script TBD.
3. **Step 4 — VE / EPI** — `[epi_ve_workflow.md](epi_ve_workflow.md)` (nucleus labels in `test_cell_labels/`):
  ```bat
...
4. **napari-feature-classifier** — installed via conda in `micro-sam-napari` (see `environment.yml`)
5. **QC** — Confirm label shape matches images; annotate ≥10 cells per class on multiple z-slices.
...
| `[micro_sam_nuclei_workflow.md](micro_sam_nuclei_workflow.md)`     | Step 2 micro-SAM nuclei                       |
